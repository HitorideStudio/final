  ...
  //jstl 혼합사용 리스트받고 푸쉬
    ...
    for(i=0;i<list.length;i+=6){
		...
		marker = new kakao.maps.Marker({
		 ...
		 //마커들 배열에 푸쉬  마커를 지울때필요
		 markers.push(marker);
		 
		 //바운즈 설정
		 bounds.extend(new kakao.maps.LatLng(list[i+2],list[i+1]));
    }
    //맵에 바운즈표현
...
function placesSearchCB(data, status, pagination) {
    ...
    list.push("${list2.placename}"); //list[i]
    list.push("${list2.lon}"); //list[i+1]
    list.push("${list2.lat}"); //list[i+2]
    list.push("${list2.address}"); //list[i+3]
    list.push("${list2.tel}"); //list[i+4]
    list.push("${list2.place}");//list[i+5]
    </c:forEach>
