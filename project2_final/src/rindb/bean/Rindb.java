...
public class Rindb {
	...
	private String rindb() throws Exception{
		...
		//csv 파일을 불러온다
<<<<<<< HEAD
		RList list = rc.eval("x <- read.csv('D:/Dropbox/R/홍대전처리완료0926.csv',stringsAsFactors = F)").asList();
=======
		RList list = rc.eval("x <- read.csv('D:/R/신촌최종.csv',stringsAsFactors = F)").asList();
